...
//    _encryWithPublicKeyTextview.text = [AES encrypt:_originTexfield.text password:_randomLabel.text];
    ...
    // 加密密文
   NSString * desString = [AES encrypt:_originTexfield.text password:_randomLabel.text];
    // 用RSA加密随机密码
    NSString * encryKey = [RSAUtil encryptString:_randomLabel.text publicKey:RSA_Public_key];
    _encryWithPublicKeyTextview.text = [NSString stringWithFormat:@"%@[]%@",encryKey,desString];
    //_encryWithPublicKeyTextview.text = [RSA encryptString: desString privateKey:RSA_Public_key];
    //_encryWithPublicKeyTextview.text = [RSAUtil encryptString: desString publicKey:RSA_Public_key];
}
...
{
    NSRange  range = [_encryWithPublicKeyTextview.text rangeOfString:@"[]"];
    // 随机字符串
    NSString * randomString = [_encryWithPublicKeyTextview.text substringToIndex:range.location];
    // 对随机字符串解密
    NSString * rsaString = [RSAUtil decryptString:randomString privateKey:RSA_Privite_key];
    // 取内容页
    NSString * conentString = [_encryWithPublicKeyTextview.text substringFromIndex:range.location];
    // 解密内容信息
    _dencryWithPriviteKeyTextview.text = [AES decrypt:conentString password:rsaString];
